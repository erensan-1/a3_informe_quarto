    ...
    for idx, row in tqdm(vc_parcela.iterrows(), total=vc_parcela.shape[0], desc=f"Procesando {nombre}"):
        ...
        if not cn_candidatos.empty:
            ...
            if not vias_cercanas.empty:
                ...
                nearest = gpd.sjoin_nearest(gdf_row, df_callejero_via_mun[['geom','codvia']], how='left', rsuffix='_right')
                if not nearest.empty:
                    df_vc.at[idx, 'cvia_ine'] = nearest['codvia'].iloc[0]
...
df_vc.loc[df_vc["a3_origen"] == 'callejero_num', "a3_metodo"] = "AsignadaViaAlternativa"
...
df_vc.to_excel("a3_asignaciones_vc_rf.xlsx", index=False)
df_vc.to_csv("a3_asignaciones_vc_rf.csv", index=False)
